...
extractRefNameFromExpr (ExprArr _ es) = (any (== True) (fst <$> tups), L.nub . L.concat $ snd <$> tups)
...
extractRefNameFromExpr (ExprDiv pos e1 e2) = extractRefNameFromExpr (ExprArr pos [e1, e2])
